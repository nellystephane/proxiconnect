    ...
    <Modal open={open} onClose={onClose} size="md">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {/* Raccourcis villes fréquentes — saisie accélérée typique du terrain */}
          <div className="flex flex-wrap gap-1.5 mt-2">
            {VILLES_SUGGESTIONS.slice(0, 6).map((v) => (
              <button
                key={v}
                type="button"
                onClick={() => { setVille(v); setError(''); }}
                className={`glass-pill rounded-full px-2.5 py-1 text-[11px] font-semibold transition-all hover:-translate-y-0.5 ${ville === v ? 'text-white' : 'text-slate-600'}`}
                style={ville === v ? { background: 'var(--gradient-primary)' } : undefined}
              >
                {v}
              </button>
            ))}
        ...
        <Textarea
          label={<>Précisions <span className="text-slate-400 normal-case">(points de repère)</span></>}
          value={details}
