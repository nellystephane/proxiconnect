...
const Profil = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-24 animate-fade-in space-y-6">
        ...
        <Card variant="glass" interactive className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center">
              <Store className="w-5 h-5 text-[#007AFF]" />
            </div>
            <div>
      ...
      {/* Onglets */}
      <div className="flex items-center gap-1 overflow-x-auto glass-light rounded-xl p-1">
        {onglets.map((o) => (
          <button
            key={o.id}
      ...
      {onglet === 'apercu' && (
        <div className="space-y-4 animate-fade-in">
          ...
          {/* Activité récente */}
          <div>
            <h2 className="flex items-center gap-2 text-sm font-bold text-slate-800 mb-3"><Activity className="w-4 h-4 text-[#007AFF]" /> Activité récente</h2>
                ...
                <ul className="divide-y divide-white/40" role="list">
                  {activiteRecente.map((item) => (
                    <li key={item.id} className="flex items-start gap-3 px-4 py-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${item.type === 'avis' ? 'bg-amber-50 text-amber-500' : 'bg-blue-50 text-[#007AFF]'}`}>
          ...
          {loadingAnnonces ? (
            ...
            <Card variant="glass-light">
              <EmptyState
                ...
                action={<Link to="/deposer" className="text-sm font-semibold text-[#007AFF] hover:underline">Déposer une annonce</Link>}
              />
            </Card>
          ) : (
            mesAnnonces.map((a) => (
