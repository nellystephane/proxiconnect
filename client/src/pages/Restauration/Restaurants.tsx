...
const Restaurants = () => {
  ...
  const fetchRestaurants = useCallback((numeroPage: number, reset: boolean) => {
    ...
    API.get('/restaurants', { params })
      .then(({ data }) => {
        setRestaurants((prev) => (reset ? data.restaurants : [...prev, ...data.restaurants]));
        setPages(data.pages);
        setPage(data.page);
      })
      .catch(() => {})
  ...
  useEffect(() => { fetchRestaurants(1, true); }, [fetchRestaurants]);
